...
public class AgentImpactVisitorIT {
    ...
    void twoAgentsShedding() {
        // given
        agentStateService.infect(data.agent2, basicConfig.WILD);
        agentStateService.progressToDiseaseStage(data.agent2, basicConfig.INFECTIOUS_SYMPTOMATIC);
        ...
        assertThat(contaminationLevel(data.householdContext1, basicConfig.WILD)).isEqualTo(2f);
        assertThat(contaminationLevel(data.householdContext2, basicConfig.WILD)).isZero();
        assertThat(contaminationLevel(data.workplace, basicConfig.WILD)).isZero();
        assertThat(contaminationLevel(data.streetsA, basicConfig.WILD)).isCloseTo(2f, VERY_CLOSE);
        assertThat(contaminationLevel(data.streetsB, basicConfig.WILD)).isCloseTo(2f, VERY_CLOSE);
        assertThat(contaminationLevel(data.streetsC, basicConfig.WILD)).isZero();
        assertThat(contaminationLevel(data.school1, basicConfig.WILD)).isEqualTo(2f);
