...
class AgentImpactVisitorTest {
    ...
    @BeforeEach
    void before() {
        agentImpactVisitor = new AgentImpactVisitor(contextsService, contextImpactService, statsService, stageImpactConfig, basicConfig.stages, "HEALTHY");
    }
    ...
    void updateImpact__no_change() {
        ...
        Entity entity = entityMocker.entity(
                impact(BehaviourType.ROUTINE, basicConfig.WILD, basicConfig.INFECTIOUS_ASYMPTOMATIC),
                behaviour(BehaviourType.ROUTINE),
                health(basicConfig.WILD, basicConfig.INFECTIOUS_ASYMPTOMATIC),
