...
class AgentImpactServiceTest {
    ...
    void updateImpact__no_change() {
        ...
        Entity entity = entityMocker.entity(
                impact(BehaviourType.ROUTINE, data.wild, Stage.INFECTIOUS_ASYMPTOMATIC),
                behaviour(BehaviourType.ROUTINE),
                health(data.wild, Stage.INFECTIOUS_ASYMPTOMATIC),
    ...
    void updateImpact() {
        ...
        Entity entity = entityMocker.entity(
                impact(BehaviourType.ROUTINE, data.wild, Stage.INFECTIOUS_ASYMPTOMATIC),
                behaviour(BehaviourType.ROUTINE),
                health(data.wild, Stage.INFECTIOUS_SYMPTOMATIC),
                person(34, Person.Sex.M));
        Context uni = ComponentCreator.context(ContextType.BIG_UNIVERSITY, 123, Map.of(data.wild, 0.1f));
        Context street = ComponentCreator.context(ContextType.STREET_20, 10.5f, Map.of(data.wild, 1.05f));
        ...
        assertThat(uni.getContaminationByLoad(data.wild).getLevel()).isEqualTo(1f);

        assertThat(street.getContaminations()).hasSize(1);
        assertThat(street.getAgentCount()).isEqualTo(10.5f);
        assertThat(street.getContaminationByLoad(data.wild).getLevel()).isEqualTo(1.5f);
    }
}
