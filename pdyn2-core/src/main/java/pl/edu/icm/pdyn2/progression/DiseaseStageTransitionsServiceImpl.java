 ...
 *
 */
public class DiseaseStageTransitionsServiceImpl implements DiseaseStageTransitionsService {
    private final SoftEnumMap<Load, LoadDiseaseStageTransitions> loadSpecificTransitions;
    private final Loads loads;

    @WithFactory
    public DiseaseStageTransitionsServiceImpl(String wildStageTransitionsFilename,
        ...
        this.loads = loads;
        load(wildStageTransitionsFilename, loads.WILD.name(), reader);
        load(alphaStageTransitionsFilename, "ALPHA", reader);
        load(deltaStageTransitionsFilename, "DELTA", reader);
        load(omicronStageTransitionsFilename, "OMICRON", reader);
        load(ba2StageTransitionsFilename, "BA2", reader);
    }

    private void load(String loadSpecificFilename, String loadName, LoadDiseaseStageTransitionsReader reader) {
        Load load = loads.getByName(loadName);
        loadSpecificTransitions.put(load, reader.readFromFile(loadSpecificFilename, load));
    }
